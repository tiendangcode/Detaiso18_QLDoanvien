...
public class StudentController {
    ...
    public StudentController(Trangchu mainview) {
       ...
       // mainview.addButtonTuyendungnhanvienListener(new TuyendungListener());
        mainview.addButtonkhenthuongListener(new SearchKhenthuongListener());
    }
    ...
    // hàm tìm kiếm nhân vien tạm thời
    class SearchKhenthuongListener implements ActionListener {

        @Override
        public void actionPerformed(ActionEvent e) {
            List<Doanvien> temp = new ArrayList<>();
            int check = mainview.loaiSearchtk();

            if (check == 1) {
                String search = mainview.ttSearch();
                // Tìm kiếm theo tên              
                temp = nhanviendao.searchByName(search);
            } 
            if (!temp.isEmpty()) {
                mainview.showtableKhenthuong(temp);
            } else {
                mainview.showMessage("Không tìm thấy kết quả!");
            }
        }
