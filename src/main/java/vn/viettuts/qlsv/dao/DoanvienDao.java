...
public class DoanvienDao {
 ...
 private Map<String, DoanvienDao> students;
        ...
        for (int i = 0; i < size; i++) {
            if (listStudents.get(i).getId().equals(student.getId())) {
                ...
                break;
                    ...
                    // Cập nhật các thông tin khác của nhân viên nếu cần
    ...
    public void sortStudentByName() {
     ...
     //sắp xếp theo Quê quán
        ...
        return result;
         ...
         public List<Doanvien> searchthongke_danhhieu(String tinhtrang) {
        List<Doanvien> result = new ArrayList<>();
        for (Doanvien nhanvien : listStudents) {
            if (nhanvien.getLop().equals(tinhtrang)) {
                result.add(nhanvien);
            }
        }
        return result;
